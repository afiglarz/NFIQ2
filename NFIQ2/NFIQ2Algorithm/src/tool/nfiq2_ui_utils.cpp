...
bool NFIQ2UI::isAN2K( const std::string& path )
{
...
bool NFIQ2UI::isAN2K( const BE::Memory::uint8Array& dataArray )
{

  // Hex representation of the string: "1.001:"
  // Each ANSI/NIST record begins with this string
  std::array<uint8_t, 6> bytes{};
  const std::array<uint8_t, 6> an2kBytes{0x31, 0x2e, 0x30, 0x30, 0x31, 0x3a};
...
bool NFIQ2UI::isRecordStore( const std::string& path )
{
...
std::string NFIQ2UI::removeSlash( std::string path )
{
...
// Yes/No prompt
bool NFIQ2UI::yesOrNo( const std::string& prompt, bool default_answer,
...
unsigned int NFIQ2UI::checkThreads( const std::string& optarg )
{
  ...
  {
    std::cerr << e.what() << "\n";
    std::cerr << "Number not given to threading flag. Single threading."
              << "\n";
    return 1;
  }
  ...
  {
    std::cerr << e.what() << "\n";
    std::cerr << "Number given to threading flag was out of range. Single "
              "threading."
              << "\n";
    return 1;
  }
  ...
  if( input > n )
  {
    const std::string prompt =
      "The requested number of threads exceeds the number of cores.\nYou "
      "currently have " +
      std::to_string( n ) +
      " CPU cores.\nWould you like to change the number of threads to " +
      std::to_string( n ) + "?\n";
  ...
  {
    std::cerr << "Non positive value given to threading flag. Single threading."
              << "\n";
    return 1;
  }
  else
...
void NFIQ2UI::printUndefinedFlag()
{
  std::cout << "\n";
  std::cout << "User Provided Undefined Flag or Defined Flag without Required "
            "Argument"
            << "\n";
